...
/*
==================================================
-- Write transformation to clean data
-- Using INSERT INTO for insert clean data to SILVER
==================================================
*/
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN UPPER (TRIM(cst_marterial_status)) = 'S' THEN 'Single'
				 WHEN UPPER(TRIM(cst_marterial_status)) = 'M' THEN 'Maried'
				 ELSE 'n/a'
		...
		FROM (
			SELECT 
			*,
			ROW_NUMBER () OVER (PARTITION BY cst_id ORDER BY cst_create_date DESC) rankrow
		...
		SELECT 
			prd_id,
		-- SUBSTRING() for extracting a specific part of a string value
		...
		CASE WHEN UPPER (TRIM(gen)) IN ('F', 'FEMALE') THEN 'Female'
			 WHEN UPPER (TRIM(gen)) IN ('M', 'MALE') THEN 'Memale'
			 ELSE 'n/a'
		END gen
